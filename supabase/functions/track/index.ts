...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!payload.session_id || !isValidUUID(payload.session_id)) {
      return new Response(JSON.stringify({ success: false, error: "Invalid session_id" }), 
        { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (!payload.event_type || !VALID_EVENT_TYPES.includes(payload.event_type)) {
      return new Response(JSON.stringify({ success: false, error: "Invalid event_type" }), 
        { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (!payload.page_url) {
      return new Response(JSON.stringify({ success: false, error: "Missing page_url" }), 
        { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } });
    }
    ...
    const sanitizedEvent = {
      ...
      screen_width: typeof payload.screen_width === "number" && payload.screen_width > 0 ? Math.floor(payload.screen_width) : null,
      screen_height: typeof payload.screen_height === "number" && payload.screen_height > 0 ? Math.floor(payload.screen_height) : null,
    ...
    const { error: insertError } = await supabaseAdmin.from("tracking_events").insert(sanitizedEvent);

    if (insertError) {
      console.error("Error inserting tracking event:", insertError);
      return new Response(JSON.stringify({ success: false, error: "Failed to record event" }), 
